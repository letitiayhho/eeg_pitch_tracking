...
def main(force, subs, skips):
    ...
    for (fpath, sub, task, run) in iter_BIDSPaths(fpaths):
        ...
        # Decode
        print('subprocess.check_call("sbatch ./decode_from_coherence_at_each_stim.py %s %s %s" % (sub, task, run), shell=True)')
        subprocess.check_call("sbatch ./decode_from_coherence_at_each_stim.py %s %s %s" % (sub, task, run), shell=True)
