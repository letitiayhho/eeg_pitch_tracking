...
def main(force, subs, skips):
    ...
    for (fpath, sub, task, run) in iter_BIDSPaths(fpaths):
        ...
        FIGS_ROOT = '../figs'
        fig_fpath = FIGS_ROOT + '/sub-' + sub + '_' + 'task-pitch_' + 'run-' + run + '_coh' + '.png'
        if os.path.isfile(fig_fpath) and sub not in subs and not force:
            print(f"Subject {sub} run {run} already decoded.")
            continue
        
        # Decode
        print('subprocess.check_call("sbatch ./decode_from_coherence.py %s %s %s" % (sub, task, run), shell=True)')
        subprocess.check_call("sbatch ./decode_from_coherence.py %s %s %s" % (sub, task, run), shell=True)
