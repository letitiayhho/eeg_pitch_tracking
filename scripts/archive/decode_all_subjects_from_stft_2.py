...
def main(subs, skips):
    ...
    for (fpath, sub, task, run) in iter_BIDSPaths(fpaths):
        ...
        # Get stft
        (Zxxs, events) = compute_stft_2.main(fpath, sub, task, run)
        
        # Decode
        subprocess.check_call("sbatch ./decode_from_stft_2.py %s %s %s %s %s" % (sub, task, run, Zxxs, events), shell=True)
